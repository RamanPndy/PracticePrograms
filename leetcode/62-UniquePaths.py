class Solution(object):
    def uniquePaths(self, m, n):
        ...
        When we're at the position [1,1] then we could get there by first moving to [0, 1] or [1, 0] so that gives us 2 options to get there so we need to sum them.
        Steps:
        1. build a 2D array to store the number of unique paths for each cell in the grid.
        2. initialize the top-left cell with a value of 1 since there is only one way to reach it.
        3. iterate through the grid row by row and calculate the number of unique paths for each cell by adding the number of unique paths from the cell above it and the cell to the left of it. 
